    ...
    public static void main(String[] args) {
        ...
        app.post("/issue", ctx -> {
            try {
                ...
                try (Connection con = DBUtil.getConnection()) {
                    // CHECK USER EXISTS
                    try (PreparedStatement checkUser = con.prepareStatement("SELECT id FROM users WHERE name=?")) {
                        checkUser.setString(1, user);
                        try (ResultSet userRs = checkUser.executeQuery()) {
                            if (!userRs.next()) {
                                ctx.result("❌ User not registered!");
                                return;
                            }
                        }
                    }
        ...
        app.post("/return", ctx -> {
            try {
                ...
                try (Connection con = DBUtil.getConnection()) {
                    LocalDate due;
                    try (Statement stmt = con.createStatement();
                         ResultSet rs = stmt.executeQuery("SELECT due_date FROM issues WHERE book_id=" + bookId + " ORDER BY id DESC LIMIT 1")) {
                        if (!rs.next()) {
